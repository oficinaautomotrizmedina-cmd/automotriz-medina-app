...
function employeeVehicleFromReception(rec) {
  const details = processRowItems(rec).map(formatProcessRow);
  ...
  return {
    ...
    detalles: details,
    detalleImages: Array.isArray(rec.trackingImages) ? rec.trackingImages : [],
...
function upsertEmployeeVehicle(vehicle) {
  ...
  setStorageSafely("am_employee_module_safe_v2", JSON.stringify(employeeState), JSON.stringify({ selected: "", seq: employeeState.seq || 0, vehicles: [compactVehicle] }));
}
...
function renderReceptionTable() {
  ...
  tbody.innerHTML = filtered.map((rec) => `
    <tr class="clickable-row ${String(rec.status || "").toUpperCase() === "FINALIZADO" ? "row-finalized" : ""}" data-open-file-row="${rec.id}" tabindex="0" title="Abrir seguimiento">
      <td data-label="Vehículo">${finalizationNeedsPublish(rec) ? `<button class="btn primary publish-finalization-btn" data-action="publish-finalization" data-id="${rec.id}" title="Publicar finalización al cliente">Publicar finalización</button>` : ""}<strong>${rec.vehicle.marca} ${rec.vehicle.modelo} ${rec.vehicle.anio}</strong>${receptionNotificationAckCount(rec) ? `<span class="vehicle-notify-count admin-vehicle-notify-count" title="Confirmaciones pendientes">${receptionNotificationAckCount(rec)}</span>` : ""}<br><small>${rec.vehicle.placa}</small></td>
...
function renderAdmin() {
  ...
  if (openClient) openClient.href = tokenHref("cliente.html", rec.clientToken);
  const openTracking = qs("[data-open-tracking]");
  if (openTracking) openTracking.href = tokenHref("seguimiento.html", rec.trackingToken);
  renderAdminFile(rec);
  renderTrackingAdmin();
  renderTabs();
  showAdminFileTab(adminFileTab);
...
function handleActions() {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (action === "sync-cloud-now") {
      ...
      AM_CLOUD_SYNC.saveNow("manual-admin").then(() => {
        cloudLog("Respaldo guardado correctamente en nube.", "ok");
      }).catch((error) => cloudLog(`No se pudo guardar en nube: ${error.message}`, "danger"));
    }
    ...
    if (action === "reactivate-reception") {
      ...
      renderAdmin();
      showSection("dashboard");
      toast("Vehículo reactivado y devuelto a taller.");
    }
    if (action === "send-global-notification") {
      ...
      renderAdmin();
      toast("Notificación enviada al empleado.");
    }
    if (action === "send-vehicle-notification") {
      ...
      AM_SIMPLE_STORE.mutate((current) => {
        ...
        ensureReceptionNotifications(rec).unshift({
          id: notificationId(),
          employeeId,
          employeeName: rec.employeeName || employeeDisplayName(employeeId),
          recId: rec.id,
      ...
      if (textField) textField.value = "";
      renderAdmin();
      toast("Notificación enviada al vehículo.");
    }
    if (action === "edit-notification") {
      ...
      const nextMessage = prompt("Editar notificación", currentMatch.item.message || "");
      if (nextMessage === null) return;
      const nextPriority = prompt("Prioridad: escriba amarillo o rojo", currentMatch.item.priority === "danger" ? "rojo" : "amarillo");
      AM_SIMPLE_STORE.mutate((current) => {
        const match = findNotificationInState(current, id);
        if (!match) return;
        match.item.message = nextMessage.trim() || match.item.message;
        match.item.priority = /rojo|danger/i.test(nextPriority || "") ? "danger" : "warn";
        match.item.editedAt = new Date().toISOString();
      });
      const updated = findNotificationInState(state(), id);
      if (updated?.rec) upsertEmployeeVehicle(employeeVehicleFromReception(updated.rec));
      renderAdmin();
    ...
    if (action === "send-client-photos") {
      ...
      renderAdmin();
      if (target) window.open(target, "_blank");
      toast("Link de revisión de fotografías generado para WhatsApp.");
    }
    if (action === "send-client-tracking") {
      ...
      renderAdmin();
      if (target) window.open(target, "_blank");
      toast("Link de seguimiento generado para WhatsApp.");
    }
    ...
    if (action === "save-progress") {
      ...
      AM_SIMPLE_STORE.mutate((current) => {
        ...
        rec.trackingImages = (draft.images || []).slice(0, rows.length);
        rec.status = profile.state || rec.status;
    ...
    if (action === "publish-pending-tracking") {
      ...
      syncSelectedAdminReceptionToEmployee();
      renderAdmin();
      toast("Seguimiento publicado al cliente.");
    }
    if (action === "publish-finalization") {
      ...
      if (updated) upsertEmployeeVehicle(employeeVehicleFromReception(updated));
      renderAdmin();
      toast("Finalización publicada al cliente.");
    }
    ...
    if (action === "admin-detail-status") {
      ...
      if (row) {
        row.dataset.detailStatus = button.dataset.status || "pending";
